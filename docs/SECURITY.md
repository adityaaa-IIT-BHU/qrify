...
| Fake job postings / employer impersonation | `Employer.verifiedStatus` field (email/domain verification) modeled in schema; UI surfacing + verification flow is P1 (see ROADMAP) — **not yet a hard gate**, flagged rather than hidden | `prisma/schema.prisma` |
...
- **Employer domain/email verification** is schema-ready (`Employer.verifiedStatus`) but not enforced as a hard gate before a job can go live — a P1 item, listed in ROADMAP, not silently dropped.
