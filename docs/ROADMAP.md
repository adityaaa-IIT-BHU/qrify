...
2. ~~**Employer verification enforced as a gate**~~ — **done.** Password-signup accounts get an `EMAIL_VERIFY` link on signup (`sendVerificationEmail`, `GET /api/auth/verify-email`, resend at `POST /api/auth/verify-email/resend`); `POST /api/jobs/[id]/qr` returns `403 unverified_employer` until the employer's `verifiedStatus` moves off `UNVERIFIED`. A banner with a resend action shows on every `/employer/*` page until verified. Magic-link/OAuth signups (verified by construction) and any employer whose creating user already had a verified email skip the gate entirely (`ensureEmployerForUser`). Verified live end-to-end: signup → job create → QR blocked (403) → verify-link consumed → QR succeeds. Domain verification (`DOMAIN_VERIFIED`, a stronger tier) is still schema-only, not built.
